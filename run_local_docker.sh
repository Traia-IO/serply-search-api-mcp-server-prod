...
# Script to build and run the Serply Search API MCP Server locally in Docker
...
IMAGE_NAME="serply-search-api-mcp-server"
CONTAINER_NAME="serply-search-api-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Serply Search API MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Serply Search API MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
if ! grep -q "^SERPLY_API_KEY=[^[:space:]]" .env; then
    if [ -t 0 ]; then
        echo ""
        echo -e "${YELLOW}📝 Please enter your Serply Search API API key:${NC}"
        echo -e "${YELLOW}   (This is the server's internal API key used when clients pay via 402)${NC}"
        read -p "SERPLY_API_KEY: " API_KEY_VALUE
        if [ -n "$API_KEY_VALUE" ]; then
            if grep -q "^SERPLY_API_KEY=" .env; then
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s|^SERPLY_API_KEY=.*|SERPLY_API_KEY=$API_KEY_VALUE|" .env
                else
                    sed -i "s|^SERPLY_API_KEY=.*|SERPLY_API_KEY=$API_KEY_VALUE|" .env
                fi
            else
                echo "SERPLY_API_KEY=$API_KEY_VALUE" >> .env
            fi
        ...
        echo -e "${YELLOW}⚠️  Non-interactive mode: Please set SERPLY_API_KEY in .env manually${NC}"
    fi
else
    echo -e "${GREEN}✅ SERPLY_API_KEY already set in .env${NC}"
fi
...
echo
echo -e "${GREEN}✅ Serply Search API MCP Server is running!${NC}"
echo
