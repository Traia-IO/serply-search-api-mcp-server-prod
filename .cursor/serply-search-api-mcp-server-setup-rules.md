# Serply Search API MCP Server Implementation Guide

You are working on implementing the Serply Search API MCP (Model Context Protocol) server. The basic structure has been set up, and your task is to implement the actual API integration.

## API Information

- **API Name**: Serply Search API
- **Documentation**: [https://serply.io/docs](https://serply.io/docs)
- **Website**: [https://api.serply.io](https://api.serply.io)
- **Authentication**: Required - API key via `SERPLY_API_KEY` environment variable
...
When a client provides their own Serply Search API API key:
...
Flow:
  1. Client connects with their Serply Search API API key
  2. MCP server uses client's API key to call Serply Search API API
  3. No payment required
  4. Client pays Serply Search API directly (not the MCP server)
```

**Use Case**: Clients who already have a Serply Search API subscription/API key
...
Flow:
  ...
  4. MCP server uses its INTERNAL Serply Search API API key to call the API
  5. Client pays the MCP server (not Serply Search API)
```

**Use Case**: Pay-per-use clients without their own Serply Search API subscription
...
async def your_tool_name(context: Context, param1: str) -> Dict[str, Any]:
    ...
    if api_key:
        ...
        api_key_to_use = os.getenv("SERPLY_API_KEY")
    
    # Call API
    headers = {"Authorization": f"Bearer {api_key_to_use}"}
    response = requests.get("https://api.serply.io/endpoint", headers=headers)
...
**Required**:
- `SERPLY_API_KEY`: Server's internal Serply Search API API key (used when clients pay via 402)
...
SERPLY_API_KEY=your_serply-search-api_api_key_here
...
```json
{
  "mcp_server": {
    "capabilities": [
      ...
      "search_serply_search_api",
      "get_serply_search_api_info",
      // Add all other tools you implement
    ]
  },
  "tags": ["serply search api", "api", /* add relevant tags like "search", "data", etc. */]
}
```

### 2. Study the API Documentation

First, thoroughly review the API documentation at https://serply.io/docs to understand:
- Available endpoints
...
Add functions to call the Serply Search API API with retry support. Example pattern:
...
def call_serply_search_api_api(endpoint: str, params: Dict[str, Any], api_key: str) -> Dict[str, Any]:
    """Call Serply Search API API endpoint with automatic retry."""
...
"Content-Type": "application/json"
    ...
    try:
        ...
        raise Exception(f"Serply Search API API error: {str(e)}")
```
...
```python
@mcp.tool()
async def search_serply_search_api(
    context: Context,
    query: str,
    limit: int = 10
) -> Dict[str, Any]:
    """
    Search Serply Search API for [specific data type].
    ...
    try:
        # The call_serply_search_api_api function already has retry logic
        results = call_serply_search_api_api(
...
```python
@mcp.tool()
async def get_serply_search_api_info(
    context: Context,
    item_id: str
) -> Dict[str, Any]:
    """
...
```python
@mcp.tool()
async def create_serply_search_api_item(
    context: Context,
    name: str,
    data: Dict[str, Any]
) -> Dict[str, Any]:
    """
    Create a new item in Serply Search API.
...
- **Log retry attempts** for debugging:
  ...
  def call_serply_search_api_api(...):
      # Implementation
  ```
...
3. **Add Tool Examples** in README.md:
   ...
   result = await tool.search_serply_search_api(query="example", limit=5)
   ```
...
### 9. Common Serply Search API Use Cases

Based on the API documentation, consider implementing tools for these common use cases:

1. **TODO**: List specific use cases based on Serply Search API capabilities
2. **TODO**: Add more relevant use cases
3. **TODO**: Include any special features of this API

### 10. Example API Calls

Here are some example API calls from the Serply Search API documentation that you should implement:
...
- Check the Serply Search API API documentation: https://serply.io/docs
- Review the MCP specification: https://modelcontextprotocol.io
- Look at other MCP server examples in the Traia-IO organization

Remember: The goal is to make Serply Search API's capabilities easily accessible to AI agents through standardized MCP tools. 