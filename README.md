# Serply Search API MCP Server

This is an MCP (Model Context Protocol) server that provides with authentication via Bearer tokens access to the Serply Search API API. It enables AI agents and LLMs to interact with Serply Search API through standardized tools.

## Features

- 🔧 **MCP Protocol**: Built on the Model Context Protocol for seamless AI integration
- 🌐 **Full API Access**: Provides tools for interacting with Serply Search API endpoints
...
- **Serply Search API Website**: [https://api.serply.io](https://api.serply.io)
- **API Documentation**: [https://serply.io/docs](https://serply.io/docs)
...
*Note: Replace `example_tool` with actual Serply Search API API tools based on the documentation.*
...
1. Clone this repository:
   ```bash
   git clone https://github.com/Traia-IO/serply-search-api-mcp-server.git
   cd serply-search-api-mcp-server
   ```

2. Set your API key:
   ```bash
   export SERPLY_API_KEY="your-api-key-here"
   ```
...
# Server's internal API key (for payment mode)
   SERPLY_API_KEY=your-api-key-here
...
2. Run the server:
   ```bash
SERPLY_API_KEY="your-api-key-here" uv run python -m server
   ```
...
Clients with their own Serply Search API API key can use the server for free:

```bash
# Request with client's API key
curl -X POST http://localhost:8000/mcp \
  -H "Authorization: Bearer CLIENT_SERPLY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"method":"tools/call","params":{"name":"example_tool","arguments":{"query":"test"}}}'
```

**Flow**:
1. Client provides their Serply Search API API key
2. Server uses client's API key to call Serply Search API API
3. No payment required
4. Client pays Serply Search API directly
...
6. Server uses its INTERNAL Serply Search API API key to call the API
...
```bash
# Required
SERPLY_API_KEY=your_internal_serply-search-api_api_key  # Server's API key (for payment mode)
...
1. **Client Decision**:
   - Has Serply Search API API key? → Mode 1 (Authenticated)
...
1. Create API client functions for Serply Search API endpoints
...
```json
{
  "github_url": "https://github.com/Traia-IO/serply-search-api-mcp-server",
  "mcp_server": {
    "name": "serply-search-api-mcp",
    "description": "Web search skill for crypto trading agents. retrieves google web search results, bing search results (including ads and shopping data), and google scholar academic papers to support market research, token discovery, and informed trading decisions.",
...
- `SERPLY_API_KEY`: Your Serply Search API API key (required)
## Troubleshooting
